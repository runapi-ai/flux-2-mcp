...
describe("flux-2 stdio MCP server", () => {
  ...
  it("exposes the model-line tools over the real stdio transport", async () => {
    ...
    client = new Client({ name: "flux-2-mcp-test", version: "0.1.0" });
    ...
    for (const model of ["flux-2-flex-remix-image","flux-2-pro-remix-image","flux-2-flex-text-to-image","flux-2-pro-text-to-image"]) {
